...
def get_config():
    ...
    config.c = c_ph
    config.seed = 0
    config.dataset = "connect-4"
    config.prior = 0.658 
    
    config.data = data =  config_dict.ConfigDict()
    data.dataset = "connect-4"
    data.source = "connect-4"
    data.target = "connect-4"
    data.uniform_dequantization = False
    # these configurations are used when train.mode = "ul_batch" 
    data.size_p = 400
    data.size_ul = 800
    data.size_u_val = 800
    # batch_size configuration is used for both source and target cases it should be divisble with jax.local_device_count()
    data.batch_size = 20  
    data.dim = 126
