...
class PhiFilteredDS:
    ...
    def _refresh_dataset(self):
        ...
        if len(accepted_x) < self.batch_size :
        ...
        if remainder > 0:
            n_pad = required - remainder
            print(f"[INFO] Randomly padding {n_pad} φ-samples to match batch shape {self.batch_dims}")

            # Random indices from the dataset (without replacement if possible)
            pad_idx = np.random.choice(n_samples, size=n_pad, replace=(n_pad > n_samples))
            pad_x = x_phi[pad_idx]

            x_phi = np.concatenate([x_phi, pad_x], axis=0)

            if y_phi is not None:
                pad_y = y_phi[pad_idx]
                y_phi = np.concatenate([y_phi, pad_y], axis=0)
