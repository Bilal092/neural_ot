...
def evaluate(config, model, state, dataset, gen_func):
    ...
    if config.data.source in ["ffhq_MAN", "ffhq_WOMAN", "ffhq_ADULT", "ffhq_YOUNG"]:  
        ...
        if config.data.source in ["ffhq_ADULT", "ffhq_YOUNG"]:
            ...
            mlp_classifier.load_state_dict(torch.load('/lustre/cniel/neural-ot-ss/lagrangian-pot-flows/subset_selection_embeddings/classifier_checkpoints/male_female_classifier.pth', map_location=DEVICE, weights_only=True))
            
            target_mlp_classifier = BinaryClassifier()
            target_mlp_classifier.load_state_dict(torch.load('/lustre/cniel/neural-ot-ss/lagrangian-pot-flows/subset_selection_embeddings/classifier_checkpoints/young_old_classifier.pth', map_location=DEVICE, weights_only=True))
        
        elif config.data.source in ['ffhq_MAN', 'ffhq_WOMAN']:
            mlp_classifier = BinaryClassifier()
            mlp_classifier.load_state_dict(torch.load('/lustre/cniel/neural-ot-ss/lagrangian-pot-flows/subset_selection_embeddings/classifier_checkpoints/young_old_classifier.pth', map_location=DEVICE, weights_only=True))
